...
The Cisco 2600 router functions as the lab gateway, receiving its upstream address via DHCP from the buffer router. The router is also prepared for secure remote management via SSH to eliminate reliance on the console cable after initial setup.
...
isolated CCNA practice environment
